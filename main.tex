...
\begin{frame}{Minimal Scope of the Proof}
    \begin{itemize}
        ...
        \vspace{0.25 cm}
        \item  Since the final theorem proves an \textit{impossibility}, if the system fails in this minimal scenario, it will inevitably fail for larger populations and more alternatives.
    \end{itemize}
\end{frame}
...
\begin{frame}{Deriving the Consequences}
    ...
    \begin{itemize}
        ...
        \vspace{0.25 cm}
        \item \textbf{Proof:} By Condition 4 (Citizens' Sovereignty), society is allowed to prefer $x'$ to $y'$ in at least one hypothetical scenario. If we take that scenario and raise $x'$ to the very top of both individuals' rankings, Condition 2 (Positive Association) dictates that society must still prefer $x'$ to $y'$. Because of Condition 3 (Independence of Irrelevant Alternatives), the choice between $x'$ and $y'$ depends only on their relative rankings, so this social preference holds \textit{whenever} both individuals agree on $x'$ over $y'$.
    \end{itemize}
\end{frame}
